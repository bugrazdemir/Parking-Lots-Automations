...
vector<vector<int>> generateParkingLot(int size) {
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            parkingLot[i][j] = rand() % 2; // Randomly generate 1 (occupied) or 0 (available)
        }
    }
    return parkingLot;
}
...
pair<pair<int, int>, int> findNearestParking(const vector<vector<int>>& parkingLot, pair<int, int> userLocation) {
    int size = parkingLot.size();
    vector<vector<bool>> visited(size, vector<bool>(size, false)); // Tracks visited locations
    queue<pair<pair<int, int>, int>> q; // Queue holding {position, distance}
