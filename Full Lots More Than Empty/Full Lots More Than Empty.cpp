...
vector<vector<int>> generateParkingLot(int size) {
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            parkingLot[i][j] = (rand() % 4 < 3) ? 1 : 0; // 75% for 1, 25% for 0
        }
    }
    return parkingLot;
}
...
pair<pair<int, int>, int> findNearestParking(const vector<vector<int>>& parkingLot, pair<int, int> userLocation) {
    int size = parkingLot.size();
    vector<vector<bool>> visited(size, vector<bool>(size, false)); // Ziyaret edilen noktalar
    queue<pair<pair<int, int>, int>> q; // {position, distance} tutan kuyruk

    // Başlangıç noktasını kuyruğa ekle
    q.push({ userLocation, 0 });
    visited[userLocation.first][userLocation.second] = true;

    // Yukarı, aşağı, sola, sağa hareket yönleri
    ...
    while (!q.empty()) {
        ...
        pair<int, int> position = currentElement.first; // Şu anki koordinat
        int distance = currentElement.second;          // Şu anki mesafe

        int y = position.first;  // Row (satır)
        int x = position.second; // Column (sütun)

        // Eğer bu nokta boş bir park yeri ise, sonucu döndür
        if (parkingLot[y][x] == 0) {
        ...
        for (auto& dir : directions) {
            ...
            // Geçerli ve ziyaret edilmemişse kuyruğa ekle
    ...
    // Eğer hiç boş park yeri bulunmazsa
    return { {-1, -1}, -1 };
}
